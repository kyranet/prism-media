...
class VolumeTransformer16LE extends VolumeTransformer {
  ...
  readInt(buffer, index) { return buffer._readInt16LE(index); }
  writeInt(buffer, int, index) { return buffer._writeInt16LE(int, index); }
}

class VolumeTransformer16BE extends VolumeTransformer {
  constructor(options, { volume = 1 } = {}) { super(options, { volume, bits: 16 }); }
  readInt(buffer, index) { return buffer._readInt16BE(index); }
  writeInt(buffer, int, index) { return buffer._writeInt16BE(int, index); }
}

class VolumeTransformer32LE extends VolumeTransformer {
  constructor(options, { volume = 1 } = {}) { super(options, { volume, bits: 32 }); }
  readInt(buffer, index) { return buffer._readInt32LE(index); }
  writeInt(buffer, int, index) { return buffer._writeInt32LE(int, index); }
}

class VolumeTransformer32BE extends VolumeTransformer {
  constructor(options, { volume = 1 } = {}) { super(options, { volume, bits: 32 }); }
  readInt(buffer, index) { return buffer._readInt32BE(index); }
  writeInt(buffer, int, index) { return buffer._writeInt32BE(int, index); }
}
